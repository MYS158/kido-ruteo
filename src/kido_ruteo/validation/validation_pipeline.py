...
def _prepare_dataframe(
    ...
    merged = df_routing.merge(df_processed, on=["origin_node_id", "destination_node_id"], how="left", suffixes=("", "_proc"))
    if df_aforo is not None and not df_aforo.empty:
        merged = merged.merge(df_aforo, on=["origin_node_id", "destination_node_id"], how="left", suffixes=("", "_aforo"))
    return merged
...
def run_validation_pipeline(
    ...
    if output_dir:
        ...
        if cfg.campos_salida.get("incluir_geojson", False) and "geometry" in result.columns:
            ...
            except Exception as exc:  # pragma: no cover - export opcional
                logger.warning("No se pudo exportar GeoJSON: %s", exc)

        if cfg.campos_salida.get("incluir_resumen", False):
            resumen = result.groupby("congruencia_nivel").size().reset_index(name="conteo")
            resumen_path = output_path / "validation_summary.csv"
            resumen.to_csv(resumen_path, index=False)
            logger.info("Resumen de validación guardado en %s", resumen_path)
